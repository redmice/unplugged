...
class Moesia_Unpluggedservices extends WP_Widget {

// constructor
    function moesia_unpluggedservices() {
		$widget_ops = array('classname' => 'moesia_unpluggedservices_widget', 'description' => __( 'Show what services you are able to provide.', 'moesia') );
        parent::__construct(false, $name = __('Moesia FP: Services', 'moesia'), $widget_ops);
		$this->alt_option_name = 'moesia_unpluggedservices_widget';	
    }
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['moesia_unpluggedservices']) )
			delete_option('moesia_unpluggedservices');		  
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'moesia_unpluggedservices', 'widget' );
		}
...
?>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'moesia_unpluggedservices', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
